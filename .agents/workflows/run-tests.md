...
### Prerequisites
- Virtual environment set up: `./llmway setup`
- API gateway running (for integration tests): `./llmway start`
- `.admin_token` file exists at project root: `./llmway key --admin`
...
// turbo
Run: `./run-tests.sh --unit` from the project root `/home/himanshu/code/llmway`.
...
```
If it returns an error, start it first: `./llmway start`
...
// turbo
Run: `./run-tests.sh --integration` from the project root `/home/himanshu/code/llmway`.
...
// turbo
Run: `./run-tests.sh` from the project root `/home/himanshu/code/llmway`.
